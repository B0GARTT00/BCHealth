...
export function DispensingPage() {
  ...
  const [form, setForm] = useState({ patientId: '', medicineBatchId: '', quantity: '1', instructions: '' });
  const dispense = useMutation({ mutationFn: () => createDispensation({ patientId: form.patientId, items: [{ medicineBatchId: form.medicineBatchId, quantity: Number(form.quantity), instructions: form.instructions }] }), onSuccess: () => { setForm({ patientId: '', medicineBatchId: '', quantity: '1', instructions: '' }); void queryClient.invalidateQueries({ queryKey: ['dispensations'] }); void queryClient.invalidateQueries({ queryKey: ['medicines'] }); } });
  ...
  const batches = medicines.data?.flatMap((medicine) => medicine.batches.filter((batch) => batch.quantity > 0).map((batch) => ({ ...batch, medicineName: medicine.name, unit: medicine.unit }))) ?? [];
