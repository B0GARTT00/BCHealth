  ...
  async function onSubmit(values: LoginForm) {
    try {
      if (isSignup) {
        ...
        setSuccessMessage(result.verificationUrl ? `${result.message} Development link: ${result.verificationUrl}` : result.message);
        return;
      } else {
    ...
    <main className="login-page relative min-h-screen overflow-hidden bg-brokenshire-900">
      ...
      <div className="relative flex min-h-screen items-center justify-center px-4 py-8 sm:px-8 lg:justify-end lg:px-[9vw]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="login-card relative w-full max-w-[440px] rounded-[22px] border p-8 shadow-2xl sm:p-10">
          ...
          {successMessage && <p className="mb-5 break-words rounded-xl border border-emerald-200/40 bg-emerald-950/40 px-3 py-2.5 text-[12px] text-emerald-100">{successMessage}</p>}
          ...
          <div className="mt-4 flex justify-center gap-2 text-[12px]"><button type="button" onClick={() => { const nextSignup = !isSignup; setIsSignup(nextSignup); setSuccessMessage(''); setError('root', { message: '' }); reset(nextSignup ? { email: '', displayName: '', password: '', confirmPassword: '' } : { email: 'admin.demo@brokenshire.edu.ph', password: 'DemoPass123!', displayName: '', confirmPassword: '' }); }} className="font-medium text-emerald-100 hover:text-white hover:underline">{isSignup ? 'Already have an account? Sign in' : 'Create an account'}</button>{!isSignup && <Link className="font-medium text-emerald-100 hover:text-white hover:underline" to="/forgot-password">Forgot your password?</Link>}</div>
