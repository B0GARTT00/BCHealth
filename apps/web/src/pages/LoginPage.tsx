    ...
    <main className="login-page relative min-h-screen overflow-hidden bg-brokenshire-900">
      ...
      <div className="relative flex min-h-screen items-center justify-center px-4 py-8 sm:px-8 lg:justify-end lg:px-[9vw]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="login-card relative w-full max-w-[440px] rounded-[22px] border p-8 shadow-2xl sm:p-10">
          ...
          {errors.root && <p className="mb-5 rounded-xl border border-rose-200/40 bg-rose-950/40 px-3 py-2.5 text-[12px] text-rose-100">{errors.root.message}</p>}
          {successMessage && <div className="mb-5 break-words rounded-xl border border-emerald-200/40 bg-emerald-950/40 px-3 py-2.5 text-[12px] text-emerald-100"><p>{successMessage}</p>{verificationUrl && <a href={verificationUrl} className="mt-2 inline-block font-semibold underline hover:text-white">Activate this development account</a>}</div>}
          {isSignup && <label className="mb-5 block"><span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-widest text-emerald-50/80">Full name</span><input autoComplete="name" className="login-input h-10 w-full rounded-xl px-3 text-[13px] outline-none transition focus:ring-2 focus:ring-emerald-200" {...register('displayName')} />{errors.displayName && <span className="text-[12px] text-rose-100">{errors.displayName.message}</span>}</label>}
          ...
          {isSignup && <label className="mb-7 block"><span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-widest text-emerald-50/80">Confirm password</span><input autoComplete="new-password" type="password" className="login-input h-10 w-full rounded-xl px-3 text-[13px] outline-none transition focus:ring-2 focus:ring-emerald-200" {...register('confirmPassword')} />{errors.confirmPassword && <span className="text-[12px] text-rose-100">{errors.confirmPassword.message}</span>}</label>}
          <button disabled={isSubmitting} className="brand-button inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl px-4 text-[13px] font-semibold text-white transition-colors disabled:opacity-60"><span>{isSubmitting ? (isSignup ? 'Creating account...' : 'Signing in...') : (isSignup ? 'Create account' : 'Sign in')}</span>{isSignup ? <UserPlus className="h-4 w-4" /> : <LogIn className="h-4 w-4" />}</button>
          ...
          {import.meta.env.DEV && <p className="mt-4 text-center text-[11px] text-emerald-50/55">Development demo: <span className="font-medium text-white/75">*.demo@brokenshire.edu.ph</span></p>}
          <div className="mt-4 flex justify-center gap-2 text-[12px]"><button type="button" onClick={() => { const nextSignup = !isSignup; setIsSignup(nextSignup); setSuccessMessage(''); setVerificationUrl(undefined); setError('root', { message: '' }); reset(nextSignup ? { email: '', displayName: '', password: '', confirmPassword: '' } : { email: 'admin.demo@brokenshire.edu.ph', password: 'DemoPass123!', displayName: '', confirmPassword: '' }); }} className="font-medium text-emerald-100 hover:text-white hover:underline">{isSignup ? 'Already have an account? Sign in' : 'Create an account'}</button>{!isSignup && <Link className="font-medium text-emerald-100 hover:text-white hover:underline" to="/forgot-password">Forgot your password?</Link>}</div>
